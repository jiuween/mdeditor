...
Lexer.prototype.lex = function(src) {
  ...
  return this.token(src, true, undefined, 0);
};
...
Lexer.prototype.token = function(src, top, bq, startline) {
  ...
  while (src) {
    ...
    // tip
    if(cap = this.rules.tip.exec(src)) {
      src = src.substring(cap[0].length);
      lined = cap[0].split('\n').length - 1;
      var level = cap[2];
      var title = level;
      if(cap[3] != '\n'){
        title = cap[3].substring(0 , cap[3].length - 1);
      }
      var text = cap[7].substring(0, cap[7].length - 1);
...
function InlineLexer(links, options) {
  ...
  this.renderer = this.options.renderer || new Renderer; //markdown 渲染器
        ...
        if (delim === 'beginend') {
          ...
          if (src.substr(0, 7) === '\\begin{') {
            ...
            var traverse = function() {
              while ((idx + 4) < src.length && src.substr(idx, 5) !== '\\end{') {
                if ((idx + 6) < src.length && src.substr(idx, 7) === '\\begin{') {
                  idx += 7;
                  traverse();
                } else {
                  idx++;
...
Renderer.prototype.code = function(code, lang, escaped) {
  ...
  if (!lang) {
    return '<pre><code class="language-sh">'
...
Renderer.prototype.tip = function(level, title, text){
  var dom = '        <div class="t-info t-' + level + '">\n' +
      '            <div class="t-info-title">' + title + '</div>\n' +
...
Parser.prototype.tok = function() {
  switch (this.token.type) {
    ...
    case 'tip':{
      return this.renderer.tip(this.token.level,
          this.token.title,
          this.token.text);
    }
    case 'table': {
